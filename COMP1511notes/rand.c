...
int main(void) {
    ...
    //Loop below frees all the memory previously allocated
...
/**
 * @brief The function to call them all
 * 
 * @param command The character representing the command
 * @param selected_shelf selected shelf
 * @param head_shelf head shelf
 * @param selected_shelf_address address of selected shelf
 * @param head_shelf_address address of head shelf
 * @return return value is not useful
 */
int command_loop(
    char command, 
    struct shelf *selected_shelf, 
...
/**
 * @brief Appends book at the end of a given shelf
 * 
 * @param head_shelf give the head of the SELECTED shelf 
 * @return returns 1 for successful appends, 0 for failures. 
 */
int append_books(struct shelf *head_shelf) {
...
/**
 * @brief Counts book on the shelf
 * 
 * @param head_shelf Pointer of the selected shelf 
 * @return Number of books on the shelf. Includes zero books too!
 */
int count_books_on_shelf(struct shelf *head_shelf) {
    struct book *book_ptr = head_shelf->books;
...
/**
 * @brief Prints books on the shelf. Invokes function print_book multiple times
 * to do so
 * 
 * @param my_shelf Shelf to print the books
 */
void print_books_on_shelf(struct shelf *my_shelf) {
    //Handle NULL books
...
/**
 * @brief Create a new empty shelf
 * 
 * @param name The name of the desired shelf
 * @return Pointer to the new shelf. 
 */
struct shelf *create_shelf(char name[MAX_STR_LEN]) {
 ...
 * @param genre Genre of type enum book_genre
 * @param rating Integer 1-5
 * @param pages_count Total pages in the book
 * @return A pointer to the allocated memory location with the book
 */
struct book *create_book(
    char title[MAX_STR_LEN],
    char author[MAX_STR_LEN],
...
/**
 * @brief Check for errors when appending new books
 * 
 * @param book_head_shelf Head of the shelf to append the book to
 * @param title Title of book
 * @param author Author of book
 * @param genre Genre of type enum book_genre
 * @param rating Rating 1-5
 * @param pages_count Total pages in book
 * @return Returns; 0 if clear, 1 if errors exits and prints an error message
 * to stdout.
 */
int check_append_errors(
    struct shelf *book_head_shelf,
...
/**
 * @brief Inserts a book at position n in the array. Note that n is taken in the
 * function instead of as a parameter.
 * 
 * @param head_shelf Pointer to the head of shelf to insert book .
 * @return Returns; 0 for successful addition attempts, 1 for failure to insert. 
 */
int insert_book(struct shelf *head_shelf) {
    int book_index;
    scanf("%d", &book_index);
...
/**
 * @brief Add pages to read_pages in any books. Handles error checking while at
 * it.
 * 
 * @param head_shelf Pointer to shelf with the book to "read" books.
 * @return Returns; 0 for successful attempts, 1 for failure.
 */
int read_book(struct shelf *head_shelf) {
    char title[MAX_STR_LEN];
    char author[MAX_STR_LEN];
...
//Finds books based on title and author
//Parameters:
//1. Pointer to Shelf
//2. Char title
//3. Char aauthor
//Returns:
//Pointer to the book in question
...
/**
 * @brief Debug function is a custom function that is not part of the assignment
 * Function debug is for developer use. Not accessible without modifying code. 
 * Go to top of program to enable DEBUG_MEM_LEAK to enable some print functions
 * and the debug function
 * 
 * @param head_shelf Address of head shelf 
 * @param selected_shelf Address of selected shelf
 */
void debug(struct shelf *head_shelf, struct shelf *selected_shelf) {
...
/**
 * @brief Inserts a new shelf ALPHABETICALLY into the linked list. Honestly
 * a work of art.
 * 
 * @param head_shelf Head shelf pointer 
 * @param head_ptr Location of head shelf pointer
 * @param name Name of new shelf
 * @return Returns: 1 for successful addition, 0 for failure. 
 */
int insert_shelf(
    struct shelf *head_shelf, 
...
/**
 * @brief Function finds shelf based on name
 * 
 * @param name_of_shelf Function of shelf to be found 
 * @param head_shelf Pointer of the head_shelf 
 * @return If successful return pointer of book, else returns NULL. 
 */
struct shelf *find_shelf(char name_of_shelf[MAX_STR_LEN], struct shelf *head_shelf) {
    while (head_shelf != NULL) {
...
/**
 * @brief Move's selected shelf left
 *
 * @param head_shelf Pointer of head shelf
 * @param current_shelf Pointer of current shelf
 * @return Returns the new position of current shelf
 */
struct shelf *move_left(struct shelf *head_shelf, struct shelf *current_shelf) {
    //If statement covers the front such that moving left when in front
    //leads to the back.
...
/**
 * @brief Move's selected shelf right
 *
 * @param head_shelf Pointer of head shelf
 * @param current_shelf Pointer of current shelf
 * @return Returns the new position of current shelf
...
/**
 * @brief Prints the shelf
 * 
 * @param shelf_head Shelf head pointer
 * @param selected_shelf Selected shelf pointer 
 */
void print_shelf(struct shelf *shelf_head, struct shelf *selected_shelf) {
...
/**
 * @brief Search ONLY the selected shelf for a particular book
 * 
 * @param selected_shelf selected shelf pointer
 * @return Returns 1 if the book can be deleted, 0 if there is no book to delete
 */
int delete_book(struct shelf *selected_shelf) {
    char author[MAX_STR_LEN];
    char title[MAX_STR_LEN];
...
/**
 * @brief Goes through the entire shelf to free books before freeing itself. 
 * Kind of romantic if you think about it. Other before itself :). 
 * 
 * @param shelf_name Pointer of shelf to be LIBERATED
 */
void free_shelf(struct shelf *shelf_name) {
...
/**
 * @brief Deletes a shelf from the linked list of shelves
 * 
 * @param head_shelf Pointer of head shelf
 * @param selected_shelf Pointer of selected shelf 
 * @param head_address Address of pointer of head 
 * @param select_address Address of pointer of selected
 * @return Returns 1 for successful shelf deletion and 0 for failure.
 */
int delete_shelf(
    struct shelf *head_shelf, 
    struct shelf *selected_shelf,
...
/**
 * @brief Create a genre shelf and appends it. 
 * 
 * @param head_shelf Pointer to head
 * @param head_ptr Address of pointer to head 
 * @return int 
 */
int create_genre_shelf(struct shelf *head_shelf, struct shelf **head_ptr) {
    //BUGS NOTE:
...
/**
 * @brief Copies book into other shelves
 * 
 * @param head_shelf Pointer of shelf to insert the book
 * @param book Pointer of book to be copied
 * @return Returns 1 for changes made, otherwise returns 0;
 */
int copy_book_onto_another_shelf(struct shelf *head_shelf, struct book *book) {
    struct book *book_ptr = head_shelf->books;
    ...
    if (head_shelf->books == NULL) {
        head_shelf->books = create_book(book->title, book->author, book->genre, book->rating, book->pages_count);
        return 1;
    } else { 
        ...
        book_ptr->next = create_book(book->title, book->author, book->genre, book->rating, book->pages_count);
        return 1;
    }
}

/**
 * @brief Takes pointer of a book and deletes it in the shelf
 * 
 * @param selected_shelf Shelf that CONTAINS the book. 
 * @param delete_ptr Pointer of book to be deleted 
 * @return Returns 1 for success, otherwise returns 0;
 */
int delete_book_by_address(struct shelf *selected_shelf, struct book *delete_ptr) {
    struct book *prev_ptr = selected_shelf->books;
    struct book *book_ptr = selected_shelf->books;
