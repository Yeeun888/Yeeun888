...
int insert_shelf(struct shelf *head_shelf, char name[MAX_STR_LEN]);
...
int command_loop(
    ...
    if (command == '?') {
        ...
        *selected_shelf_address = move_right(head_shelf, selected_shelf);
        ...
        delete_shelf(head_shelf, selected_shelf, head_shelf_address, selected_shelf_address);
    } else if(command == 'z') {
...
int check_append_errors(
    ...
    while (book_ptr != NULL) {
        ...
        strcmp(author, book_ptr->author) == 0
            ...
            printf("ERROR: a book with title: '%s' by '%s' already exists in this shelf\n", title, author);
            return 1;
        }
        book_ptr = book_ptr->next;
    }
...
int insert_shelf(struct shelf *head_shelf, char name[MAX_STR_LEN]) {
    ...
    if(strcmp(name, head_shelf->name) < 0) {
        struct shelf *temp_shelf = malloc(sizeof(struct shelf));
        *temp_shelf = *head_shelf;

        struct shelf *new_head_shelf = malloc(sizeof(struct shelf));
        new_head_shelf->books = NULL;
        new_head_shelf->next = temp_shelf;
        strcpy(new_head_shelf->name, name);
        *head_shelf = *new_head_shelf;
        return 1;
    }
    ...
    head_shelf->next = malloc(sizeof(struct shelf));
    strcpy(head_shelf->next->name, name);
    head_shelf->next->books = NULL;
    head_shelf->next->next = NULL;
    return 1;
}
